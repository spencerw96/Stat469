...
```{r}
#According to our graph above, viewership appears to be increasing. We will
#verify with a hypothesis test and confidence interval.
...
```{r}
pred_view <- max(tv$SeasEp) + seq(1/10, 10*(1/10), by=1/10)
Xpred <- predict(X, newx=pred_view)
my_for <- sarima.for(climate$AnnAnom, xreg=X,
                     p=2, d=0, q=0, P=0, D=1, Q=1, S=10,
                     n.ahead=10, newxreg=Xpred)
```
